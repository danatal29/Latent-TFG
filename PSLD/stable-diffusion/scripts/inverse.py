...
def main():
    ...
    if hasattr(model, "cond_stage_model") and model.cond_stage_model is not None:
        ...
        # keep a helper attribute so other code can read it
        try:
            model.cond_stage_model.device = device
        except Exception:
            pass

    print("UNet:", next(model.parameters()).device)
    print("CLIP:", next(model.cond_stage_model.parameters()).device)
    print("attr:", getattr(model.cond_stage_model, "device", None))
    ...
    x_checked_image_torch = img[:,:3,:,:].to(device)
    ...
    #pdb.set_trace()
    use_amp = (opt.precision == "autocast")
    with amp_context(device=device, use_amp=use_amp):
