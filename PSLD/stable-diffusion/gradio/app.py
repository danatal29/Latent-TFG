...
## device util
def amp_context(device, precision_flag=True):
    # Only use autocast when explicitly requested AND it makes sense.
    if not precision_flag:
        return nullcontext()
    if device.type == "cuda":
        # CUDA autocast is stable with fp16
        return torch.autocast(device_type="cuda", dtype=torch.float16)
    if device.type == "cpu":
        # CPU autocast supports bfloat16; optional
        return torch.autocast(device_type="cpu", dtype=torch.bfloat16)
    # MPS: prefer fp32 for stability (skip autocast)
    return nullcontext()
...
def predict(ddim_steps, gamma, gluing_kernel_size, gluing_kernel_sigma, omega, dict, prompt=""):
    ...
    with amp_context(device, precision_flag=(opt.precision == "autocast")):
